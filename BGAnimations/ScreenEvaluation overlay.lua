        ...
        local function makejudge(i)
                return LoadFont("Common Normal") .. {
                        InitCommand = function(self)
                                ...
                                self:settextf("%s: %d",
                                              getJudgeStrings(judges[i]),
                                              score:GetTapNoteScore(judges[i]))
                        end
                }
        end
        for k, v in ipairs(judges) do t[#t + 1] = makejudge(k) end
        return t
end
...
t[#t + 1] = Def.ActorFrame {
        ...
        makejudges() .. {InitCommand = function(self) self:y(50) end},
        LoadFont("Common Normal") .. {
                InitCommand = function(self)
                        self:y(30)
                        self:settextf("%5.2f --- %5.2f",
        ...
        LoadFont("Common Normal") .. {
                InitCommand = function(self)
                        self:y(5):valign(0)
                        self:settext(score:GetModifiers() .. ", Judge " ..
                                         GetTimingDifficulty())
                        self:zoom(.4)
                end
        },
