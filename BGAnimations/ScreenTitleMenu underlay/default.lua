return Def.ActorFrame {
        ...
        LoadActor("_scroller_bg.png") .. {
                InitCommand = function(self)
                        ...
                        self:draworder(100)
                        self:x(offset)
                        self:zoomto(SCREEN_WIDTH + SAFE_WIDTH + offset,
        ...
        LoadActor("_scroller_bg.png") .. {
                InitCommand = function(self)
                        ...
                        self:x(offset)
                        self:zoomto(
                            SCREEN_WIDTH + SAFE_WIDTH + math.abs(offset),
                            SCREEN_HEIGHT + SAFE_HEIGHT)
                        self:diffusealpha(0.9)
                end
        },
        ...
        Def.BitmapText {
                ...
                UpdateTextCommand = function(self)
                        self:settext("Quark r" .. ThemeInfo.Release .. " (" ..
                                         ThemeInfo.Codename .. ")\nMade by " ..
                                         ThemeInfo.Author)
                end,
                CodeMessageCommand = function(self, params)
                        if params.Name == "EnableDebug" then
                                if ReadGamePrefFromFile("Quark_DebugEnabled") ~=
                                    "true" then
                                        WriteGamePrefToFile(
                                            "Quark_DebugEnabled", "true")
                                        SCREENMAN:SystemMessage(
                                            "You are now a developer!\nRestart to apply changes.")
                                else
                                        WriteGamePrefToFile(
                                            "Quark_DebugEnabled", "false")
                                        SCREENMAN:SystemMessage(
