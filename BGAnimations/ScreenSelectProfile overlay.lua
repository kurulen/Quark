        ...
        for p = 0, PROFILEMAN:GetNumLocalProfiles() - 1 do
                ...
                local ProfileCard = Def.ActorFrame {
                        ...
                        LoadFont("Common Normal") .. {
                                InitCommand = function(self)
                                        self:y(8):zoom(0.5):vertspacing(-8)
                                            :ztest(true):maxwidth(
                                                (200 - 34 - 4) / 0.5)
                                end,
                                BeginCommand = function(self)
                                        local numSongsPlayed =
                                            profile:GetNumTotalSongsPlayed()
                                        local s =
                                            numSongsPlayed == 1 and
                                                translated_info["SongPlayed"] or
                                                translated_info["SongsPlayed"]
...
local function LoadPlayerStuff(Player)
        ...
        t[#t + 1] = Def.ActorScroller {
                ...
                NumItemsToDraw = 6,
                OnCommand = function(self)
                        self:y(1):SetFastCatchup(true):SetMask(200, 58)
                            :SetSecondsPerItem(0.15)
                end,
...
local function input(event)
        if event.type == "InputEventType_FirstPress" then
                ...
                elseif event.button == "Down" or event.button == "MenuDown" then
                        local ind = SCREENMAN:GetTopScreen():GetProfileIndex(
                                        PLAYER_1)
                        if ind > 0 then
                                if SCREENMAN:GetTopScreen():SetProfileIndex(
                                    PLAYER_1, ind + 1) then
                                        MESSAGEMAN:Broadcast("DirectionButton")
                                        theThingVeryImportant:queuecommand(
                                            "UpdateInternal2")
                                end
                        end
...
t[#t + 1] = Def.ActorFrame {
        InitCommand = function(self) theThingVeryImportant = self end,
        OnCommand = function(self)
                ...
                Def.ActorFrame {
                        ...
                        PlayerJoinedMessageCommand = function(self, param)
                                if param.Player == PLAYER_1 then
                                        self:zoom(1.15):bounceend(0.175):zoom(
                                            1.0)
                                end
                        end,
