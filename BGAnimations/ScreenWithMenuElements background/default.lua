...
t[#t+1] = Def.ActorFrame {
  ...
  LoadActor("_horzstripe.png") .. {
     InitCommand=function(self)
	self:zoomto(SCREEN_WIDTH+SAFE_WIDTH,SCREEN_HEIGHT+SAFE_HEIGHT)
	self:texcoordvelocity(0,-0.3)
     end
			      },
  LoadActor("_bg_over.png") .. {
     InitCommand=function(self)
	self:zoomto(SCREEN_WIDTH*2+SAFE_WIDTH*2,SCREEN_HEIGHT*2+SAFE_HEIGHT*2)
	self:z(-256)
     end
			   },
  ...
  LoadActor("_bg.png") .. {
     InitCommand=function(self)
	self:zoomto(SCREEN_WIDTH*3+SAFE_WIDTH*2,SCREEN_HEIGHT*3+SAFE_HEIGHT*2)
	self:z(-512)
